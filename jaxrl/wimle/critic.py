...
def update_quantile(key: PRNGKey, actor: Model, quantile_critic: Model, target_quantile_critic: Model,
           temp: Model, batch: Batch, discount: float, taus: jnp.ndarray) -> Tuple[Model, InfoDict]:
    kappa = 1.0
    ...
    def critic_loss_fn(quantile_critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        critic_loss = calculate_quantile_huber_loss(td_errors1, taus, batch.coefs, kappa=kappa) + calculate_quantile_huber_loss(td_errors2, taus, batch.coefs, kappa=kappa)
        return critic_loss, {
...
def calculate_quantile_huber_loss(td_errors, taus, coefs, kappa: float = 1.0):
    ...
    quantile_huber_loss = element_wise_quantile_huber_loss.sum(axis=1).mean(axis=[1]) * coefs
